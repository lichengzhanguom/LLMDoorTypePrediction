...
for line in lines:
    ...
    try:
        ...
        continue
    f.write(name + ':')
    boxes = name_box[1].strip(';').split(';')
    width_height_list = list()
    for box in boxes:
        x1, y1, x2, y2, _, _ = box.split(',')
    ...
    for i, box in enumerate(boxes):
        ...
        width = x2 - x1
        height = y2 - y1
        if width * height <= mean_width and (width / height > 2 or height / width > 2):
            print(name, i)
            continue
        f.write(box+';')
    f.write('\n')
